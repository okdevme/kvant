...
interface InteractiveHeroCanvasProps {
  ...
  lineWidth?: number
  lineCount?: number
  lineLength?: number | ((i: number, lineCount: number) => number)
  scaleX?: number
  speed?: number
  glow?: number
  interactive?: boolean
...
export default function InteractiveHeroCanvas({
  ...
  const { containerRef, canvasRef, stateRef } = useCanvasAnimation({
    ...
    onDraw: (e, t) => {
      const isMobile = t.width < 768
      const sX = isMobile ? scaleX * 0.7 : scaleX

      const { width: r, height: i, frameCount: fc } = { ...t, width: t.width * (1 / sX) }
      ...
      e.translate(r / (2 / sX) + (offset?.x ?? 0), i / 2 + (offset?.y ?? 0))
      e.scale(sX, 1)

      const f = interactive ? map(mouse.y, 0, i, 1.2, -1.2) * 0.2 : 0
      ...
      for (let k = 0; k < x; k++) {
        ...
        const rawLen
          = typeof lineLength === 'function' ? lineLength(k, x) : lineLength
        const len = Math.max(0, Math.min(1, rawLen))
        if (!(len > 0))
          continue
